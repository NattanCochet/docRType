...
{
    public:
        enum STATE { NOTHING = 0, HOLD = 1, PLAY = 2 };
        /**
         ...
         */
        World(bool client, bool isPublic, int nbrPlayerMax, std::string nameRoom, std::size_t clientID,
            std::string password = "");
        ...
        const std::size_t getNbrClientInWorld() const noexcept;

        const std::size_t getNbrClientMaxInWorld() const noexcept;
         ...
         */
        void resetFinishedLevel(bool isWin, NetworkServer &server);
        ...
        void applyFonctionInWorld(NetworkServer &server);
        ...
        const bool isAllClientReady() noexcept;

        void sendToAllClientEntityDead(
    ...
    private:
        ...
        static constexpr float SPAWN_TIME_THRESHOLD =
            19.0f; ///< A Contante expression for the second elapsed to generate world
        static constexpr std::size_t SPAWN_AFTER_WINDOW =
            400; ///< The value to spawn after the window (_widthWindow + SPAWN_AFTER_WINDOW)
    ...
    private:
         ...
         */
        std::vector<std::vector<int>> generateWorldWithID(std::size_t level,
         ...
         */
        std::vector<std::vector<int>> generateWorldWithID(std::size_t level,
            std::pair<int, int> &beginAndEndGenerationInX, std::pair<int, int> &beginAndEndGenerationInY);
         ...
         */
        std::vector<std::vector<SpawnRule::Generator::ENTITIES>> generateWorldWithEnum(std::size_t level,
            std::pair<int, int> &beginAndEndGenerationInX, std::pair<int, int> &beginAndEndGenerationInY);
        ...
        std::size_t generateRandomValueForEntityAfterDead(std::size_t seed) noexcept;
};
